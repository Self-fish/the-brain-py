...
class HandleLightsUseCase:
    ...
    def handle_lights(self):
        ...
        LogsApiDataSource.log_info("HandleLights: checking if lights should be on/off at " + current_time)
        preferences = Preferences.get_light_preferences()
        if should_turn_on_lights(current_time, preferences):
            LogsApiDataSource.log_info("HandleLights: lights must be on")
            self.__light_repository.update_light_status(RelayStatus.ON)
        else:
            LogsApiDataSource.log_info("HandleLights: lights must be off")
